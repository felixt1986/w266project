...
def embedding_layer(ids_, V, embed_dim, init_scale=0.001):
    ...
    W_embed_ = tf.get_variable("W_embed", shape = (V,embed_dim), initializer = tf.random_uniform_initializer(-init_scale, init_scale))
...
def fully_connected_layers(h0_, hidden_dims, activation=tf.tanh,
    ...
    for i, hdim in enumerate(hidden_dims):
        ...
        if dropout_rate > 0:
            h_ = tf.nn.dropout(h_, keep_prob = 1 - dropout_rate)  # replace with dropout applied to h_
...
def softmax_output_layer(h_, labels_, num_classes):
    ...
    with tf.variable_scope("Logits"):
        #### YOUR CODE HERE ####
        W_out_ = tf.get_variable("W_out", shape = (h_.shape[1], num_classes), initializer = tf.random_normal_initializer())
    ...
    with tf.name_scope("Softmax"):
        #### YOUR CODE HERE ####
        loss_ = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(labels = labels_, logits = logits_), axis=0) # replace with mean cross-entropy loss over batch
...
def BOW_encoder(ids_, ns_, V, embed_dim, hidden_dims, dropout_rate=0,
    ...
    summed_embed_ = tf.reduce_sum(xs_, axis=1)

    # Build a stack of fully-connected layers
    h_ = fully_connected_layers(summed_embed_, hidden_dims, activation = tf.tanh, dropout_rate=0, is_training=False)
#     fully_connected_stack_ = fully_connected_layers(h0_, hidden_dims, activation=tf.tanh, dropout_rate=0, is_training=False)
