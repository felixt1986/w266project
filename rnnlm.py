    ...
    def BuildCoreGraph(self):
        ...
        with tf.name_scope("Embedding_Layer"):
            ...
            self.x_ = tf.reshape(tf.nn.embedding_lookup(self.W_in_, self.input_w_), 
                                 [self.batch_size_, self.max_time_, self.H], name="x")
        ...
        with tf.name_scope("Recurrent_Layer"):
            ...
            self.output_, self.final_h_ = tf.nn.dynamic_rnn(cell=self.cell_, inputs=self.x_, 
        ...
        with tf.name_scope("Scoring_Loss_Function"):
            self.per_example_loss_ = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.target_y_,logits=self.logits_, name="per_example_loss")
...
#             print(self.train_loss_)
            

        # Define optimizer and training op
        with tf.name_scope("Training_Op"):
            
            self.train_step_ = tf.train.AdamOptimizer(self.learning_rate_).minimize(self.train_loss_)
